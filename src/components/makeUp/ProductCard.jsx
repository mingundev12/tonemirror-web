...
export default function ProductCard({  userToneStatus, products }) {
    return (
        <div className="glass w-full h-full min-w-0 rounded-3xl p-8 border border-[#FDFAF7]/30 flex flex-col gap-4">
            ...
            <div>
                <p className="text-lg text-[#3D2E35] font-gmarket">{productCardConst[0]}</p>
                <p className="text-sm font-light text-[#3D2E35]/60 font-gmarket">{userToneStatus}{productCardConst[1]}</p>
            </div>
                    ...
                    <div key={item.brand} className="flex items-center gap-4 rounded-2xl border border-[#FDFAF7]/30 p-4 flex-1 glass">
                        ...
                        <div className="flex flex-col gap-1">
                            ...
                            <div className="flex items-center gap-2">
                                <span className="font-light text-[#3D2E35]/50 md:text-base text-xs font-gmarket">{item.shadeName} / {item.shade}{productCardConst[2]}</span>
                            </div>
                        </div>
                    </div>
                ))}
