...
export default function MakeUpContent({motion, products, userToneStatus, beforeSrc, afterSrc, isRecoloring, onSelectFoundation}) {
   return (
       <>
           <div className="flex md:grid flex-col md:grid-cols-[2.5fr_7.5fr] items-stretch gap-5 w-full h-auto">
                {/* 메이크업 비교 (before: 업로드 원본 / after: 서버 합성 이미지) */}
                <motion.div className="w-full h-auto shrink-0" initial={{ y: 200, opacity: 0}} whileInView={{ y: 0, opacity: 1}} viewport={{ once: true, amount: 0.3 }} transition={{ duration: 0.1, delay: 0.1, ease: "easeInOut", type: "spring", damping: 10, stiffness: 50}}>
